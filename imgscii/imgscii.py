...
def display_ascii(ascii_list, display):
    ...
    display.configure(text=ascii_text)
...
def read_pixel_data(img, monochrome, width=60, char_set=ASCII_CHARS):
    ...
    for pixel in pixels:
        ...
        color = get_color(pixel, monochrome)
...
def get_color(pixel, monochrome):
...
def printscii(image, display, monochrome=False):
    ...
    try:
        ...
        ascii_image = read_pixel_data(image, monochrome, width, characters)
        display_ascii(ascii_image, display)
